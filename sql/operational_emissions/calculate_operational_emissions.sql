...
UPDATE cbs_map_all
SET emissions_kg_gas = g_gas_tot * 1.9,
    emissions_kg_electricity = g_elek_tot * 0.45,
    emissions_kg_total = (g_gas_tot * 1.9) + (g_elek_tot * 0.45),
    emissions_kg_pp = CASE 
                        WHEN aant_inw <= 0 THEN NULL
                        ELSE ((g_gas_tot * 1.9) + (g_elek_tot * 0.45)) / aant_inw 
                      END;

...
class QueryRunner(): 
    ...
    def run_query_for_cbs_map_all_wijk(self, message=''):
        if message != '': 
            print(message)
        for i, municipality in enumerate(self.municipalities):
            for year in range(2012, 2022): 
                output = f"\rProcessing year {year} for municipality ({i+1}/{len(self.municipalities)}): {municipality}                         "
                sys.stdout.write(output)
                sys.stdout.flush()
                self.cursor.execute(self.query, (year, municipality, municipality, year, year))
                self.conn.commit()
        print('\nDone!\n')
    ...
    def run_query_to_combine_emissions(self, message=''):
        ...
        for i, municipality in enumerate(self.municipalities): 
            for year in range(2012, 2025): 
                output = f"\rCombining emissions for ({i+1}/{len(self.municipalities)}): {municipality} for year {year}                         "
                sys.stdout.write(output)
                sys.stdout.flush()
